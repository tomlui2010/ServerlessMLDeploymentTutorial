...
```bash
serverless-ml-deployment$ sam build
```
...
```bash
serverless-ml-deployment$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless-ml-deployment$ sam local start-api
serverless-ml-deployment$ curl http://localhost:3000/classify_digit
```
...
```bash
serverless-ml-deployment$ sam logs -n InferenceFunction --stack-name "serverless-ml-deployment" --tail
```
...
```bash
sam delete --stack-name "serverless-ml-deployment"
```
